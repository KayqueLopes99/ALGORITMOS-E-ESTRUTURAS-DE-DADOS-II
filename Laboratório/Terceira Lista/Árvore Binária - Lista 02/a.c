...
int main()
{
    ...
    info[2] = criarInfo("Daniel_Costa", 5, 1, rand() % 10);
    info[3] = criarInfo("Isabelly", 6, 1, rand() % 10);
    info[2] = criarInfo("Leonardo", 2, 1, rand() % 10);
    info[3] = criarInfo("Letícia", 9, 1, rand() % 10);
    ...
    raiz = inserir(raiz, info[4]);
    raiz = inserir(raiz, info[5]);
}
